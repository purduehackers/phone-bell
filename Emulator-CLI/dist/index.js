...
class PhoneBellEmulator {
    ...
    clientId;
    peers = new Map(); // clientId -> iroh nodeId
    ...
    handlePhoneControlMessage(message) {
        switch (message.type) {
            case "Ring":
    ...
    sendPhoneControlMessage(message) {
    ...
    async connectToPeer(nodeId) {
        if (!this.endpoint || this.isConnectingToPeer)
            return;
        if (this.connection) {
    ...
    showStatus() {
        ...
        console.log(`Iroh Node ID: ${this.endpoint?.nodeId().slice(0, 16) || "N/A"}...`);
        ...
        for (const [clientId, nodeId] of this.peers) {
            console.log(`  - ${clientId.slice(0, 8)}... -> ${nodeId.slice(0, 16)}...`);
        }
        console.log(`Phone Control WS: ${this.phoneControlWs?.readyState === WebSocket.OPEN ? "Connected" : "Disconnected"}`);
        console.log(`Signaling WS: ${this.signalingWs?.readyState === WebSocket.OPEN ? "Connected" : "Disconnected"}`);
        console.log("--------------");
    }
    quit() {
        console.log("\nGoodbye! 👋");
...
// Main
const phoneType = process.argv[2] || "outside";
